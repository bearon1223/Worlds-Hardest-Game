...
void coin(float x, float y, float coinAmount, float currentCoint) {
  float ca = floor(coinAmount);
  boolean currentCointCollected = false;
  if (coinsCollected > currentCoint) {
    currentCointCollected = true;
  } else {
    currentCointCollected = false;
  }

  if (!currentCointCollected) {
    stroke(1);
    strokeWeight(1);
    fill(255, 255, 0);
    ellipse((x * 25) + (25 / 2), (y * 25) + (25 / 2), 15, 15);
  }
  if (coinsCollected == ca) {
    allowd = true;
  }
